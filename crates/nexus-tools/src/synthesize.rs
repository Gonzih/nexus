...
/// Nexus_ARG_<NAME>=<value> for each arg in the schema.
...
impl Tool for SynthesizedShellTool {
    ...
    async fn execute(
        ...
        // Build env vars from arguments: Nexus_ARG_<KEY>=<val>
        let mut cmd = tokio::process::Command::new("bash");
        cmd.arg("-c").arg(&self.script);

        if let Some(obj) = arguments.as_object() {
            for (k, v) in obj {
                let env_key = format!("Nexus_ARG_{}", k.to_uppercase().replace('-', "_"));
                let env_val = match v {
    ...
    pub fn new(handle: DynamicToolHandle) -> Self {
        let definition = ToolDefinition {
            ...
            description: concat!(
                ...
                "The script receives arguments as env vars: Nexus_ARG_<NAME>=<value>. ",
            ...
            input_schema: json!({
                ...
                "properties": {
                    ...
                    "script": {
                        "type": "string",
                        "description": "Bash script body. Access args via $Nexus_ARG_<NAME>."
                    },
                    "args_schema": {
                        "type": "object",
        ...
        let result = synth
            .execute(
                ...
                json!({
                    ...
                    "script": "echo \"Hello, $Nexus_ARG_USER\"",
                    "args_schema": {
                        "type": "object",
