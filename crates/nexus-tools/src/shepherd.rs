...
/// Nexus Shepherd tool — spawn and manage agent sessions via the Shepherd HTTP API.
///
...
impl Tool for ShepherdTool {
    ...
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            name: "shepherd".into(),
            description: r#"Spawn and manage AI agent sessions via the Nexus Shepherd API.
...
1. (Optionally) post a contract via nexus_contracts first, get contract_id
2. shepherd spawn(task_prompt, contract_id?) → session_id
3. Loop: shepherd poll_result(session_id) every 30s until done=true
4. (If used contract) nexus_contracts settle(fulfilled/disputed)
"#
            .into(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    ...
                    "binary_path": {
                        "type": "string",
                        "description": "Path to nexus binary on the host (uses default if omitted)"
                    },
                    "config_path": {
                        "type": "string",
        ...
        let result = match action {
            "spawn" => {
                ...
                let binary_path = arguments
                    .get("binary_path")
                    .and_then(|v| v.as_str())
                    .unwrap_or("/Users/boxy/nexus-infra/nexus/target/release/nexus");
                let config_path = arguments
                    .get("config_path")
                    .and_then(|v| v.as_str())
                    .unwrap_or("/Users/boxy/nexus-infra/nexus/crates/nexus-agent/agent-shepherd-anthropic.toml");
                let cwd = arguments
                    .get("cwd")
                    .and_then(|v| v.as_str())
                    .unwrap_or("/Users/boxy/nexus-infra/research");
                    ...
                    .map(|cid| {
                        format!(
                            "\n\nYou are operating under contract {cid}. \
                             IMPORTANT: Your first action must be to call nexus_contracts accept(contract_id=\"{cid}\") \
                             ...
                             If you cannot complete the work, call nexus_contracts settle(contract_id=\"{cid}\", outcome=\"breached\", note=\"<reason>\")."
                        )
                    })
                    .unwrap_or_default();
