...
pub struct IdentityConfig {
    ...
    /// Directory for identity keys (default: ~/.amai/identity/)
...
pub struct SoullogConfig {
    ...
    pub url: String,
    /// Service name in soullog events (default: amai-agent)
...
pub struct LoggingConfig {
    /// Directory for session logs (default: .amai-logs/)
...
impl AgentToml {
    ...
    /// Load from default locations: ./amai-agent.toml, ~/.config/amai/agent.toml
    pub fn load_default() -> Result<Self, String> {
        let candidates = [
            "amai-agent.toml".to_string(),
            dirs_candidate(),
        ];
...
fn dirs_candidate() -> String {
    if let Some(home) = std::env::var_os("HOME") {
        format!("{}/.config/amai/agent.toml", home.to_string_lossy())
    } else {
        "~/.config/amai/agent.toml".into()
    }
}
...
"#;
    ...
    fn logging_config_defaults() {
        let toml_str = "";
        let config: AgentToml = toml::from_str(toml_str).unwrap();
        assert_eq!(config.logging.dir, ".amai-logs");
...
"#;
        ...
        assert_eq!(sl.service, "amai-agent");
