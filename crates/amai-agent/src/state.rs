...
impl SessionState {
    ...
    /// Find the most recently modified state file in .amai-state/
...
mod tests {
    ...
    fn state_dir_is_under_cwd() {
        ...
        assert_eq!(dir, PathBuf::from("/tmp/project/.amai-state"));
    }
    ...
    fn state_path_includes_session_id() {
        ...
        assert_eq!(
            path,
            PathBuf::from("/tmp/project/.amai-state/session-123.json")
        );
    }
