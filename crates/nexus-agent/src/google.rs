...
/// Default credentials path: ~/.nexus/google/credentials.json
fn default_credentials_path() -> Option<std::path::PathBuf> {
    dirs::home_dir().map(|h| h.join(".nexus/google/credentials.json"))
}

/// Default tokens path: ~/.nexus/google/tokens/default.json
fn default_tokens_path() -> Option<std::path::PathBuf> {
    dirs::home_dir().map(|h| h.join(".nexus/google/tokens/default.json"))
}
