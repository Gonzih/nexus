...
//! 5. Persists skills to `.nexus-skills/` for auto-reload next session
...
///
/// `https://id.nexus.net/skill.md` → `"id_nexus_net"`
...
///
/// `("id_nexus_net", "register_identity")` → `"id_nexus_net__register_identity"`
///
...
impl Tool for InstallSkillTool {
    ...
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            description: "Install skills from a URL or raw content. Accepts BOTH: \
                ...
                Skills persist to .nexus-skills/ and auto-load next session."
                .into(),
            input_schema: json!({
                "type": "object",
                "properties": {
    ...
    async fn execute(
        ...
        // Extract namespace from URL hostname (e.g. "id_nexus_net" from "https://id.nexus.net/...")
                ...
                return match self.register_skill(&raw_content, ns) {
                    Ok(name) => {
                        ...
                        Ok(ToolOutput::success(format!(
                            "Skill '{name}' installed (persisted to .nexus-skills/).\n\
        ...
        let mut result = format!(
            "Auto-generated and installed {} skills from documentation (persisted to .nexus-skills/):\n",
            installed.len()
        );
...
async fn fetch_url(url: &str) -> Result<String, String> {
    ...
    let response = client
        .get(url)
        .header("User-Agent", "nexus-agent/0.1")
        .send()
        .await
...
"#;
    ...
    fn namespace_from_url_extracts_host() {
        assert_eq!(
            namespace_from_url("https://id.nexus.net/skill.md"),
            Some("id_nexus_net".into())
        );
        assert_eq!(
    ...
    fn namespaced_name_prefixes_correctly() {
        assert_eq!(
            namespaced_name("id_nexus_net", "register_identity"),
            "id_nexus_net__register_identity"
        );
    }
    ...
    fn namespaced_name_avoids_double_prefix() {
        assert_eq!(
            namespaced_name("id_nexus_net", "id_nexus_net__register_identity"),
            "id_nexus_net__register_identity"
        );
    }
...
---
"#;
        let name = tool.register_skill(skill_md, Some("id_nexus_net")).unwrap();
        assert_eq!(name, "id_nexus_net__health_check");
        assert_eq!(handle.len(), 1);
    }
