/// The Nexus SWE system prompt — designed for weaker models operating autonomously.
///
/// Design principles:
...
pub const SYSTEM_PROMPT: &str = r#"You are Nexus, an autonomous software engineering agent.
...
You can create and install new tools at runtime using `install_skill`. Skills persist to `.nexus-skills/` and auto-load on next session.
...
3. Skills persist to .nexus-skills/ and auto-load next session
