...
use nexus_tools::{ContractsTool, ShepherdTool, agent_tools_vec};

#[derive(Parser)]
#[command(name = "nexus", about = "Nexus autonomous coding agent")]
struct Cli {
    /// Task to execute (prompt)
    #[arg(trailing_var_arg = true)]
    task: Vec<String>,

    /// Config file path (default: nexus-agent.toml)
    #[arg(short, long)]
...
async fn main() {
    ...
    let task = if cli.task.is_empty() {
        if cli.benchmark {
            ...
            eprintln!("Usage: nexus <task>");
            eprintln!("       nexus --telegram                             (Telegram gateway mode)");
            eprintln!("       nexus --shepherd ws://host:8084/ws/sessions/ID  (Shepherd managed mode)");
            eprintln!("       nexus --supervisor --telegram                 (supervised auto-restart)");
            eprintln!("       nexus --benchmark [task]");
            eprintln!("       nexus --synthetic");
            eprintln!("Example: nexus \"fix the failing test in src/lib.rs\"");
            std::process::exit(1);
        }
    } else {
        cli.task.join(" ")
...
async fn run_agent_task_with_event_tx(
    ...
    let skills_dir = std::path::Path::new(cwd).join(".nexus-skills");
    ...
    // Nexus contracts tool: register if identity is available
...
async fn run_agent_task(
    ...
    // Load persisted skills from .nexus-skills/
    if skills_dir.exists() {
        load_persisted_skills(&skills_dir, &tool_registry);
    }
...
async fn run_single_task(
    ...
    if !context_files.is_empty() {
        let ctx_dir = std::path::Path::new(cwd).join(".nexus-context");
            ...
            match std::fs::read_to_string(ctx_path) {
                Ok(content) => {
                    ...
                    context_note.push_str(&format!(
                        "\nContext file staged: .nexus-context/{filename} — read it on your first turn."
                    ));
                    tracing::info!(
                        source = %ctx_path.display(),
                        dest = %format!(".nexus-context/{filename}"),
                        bytes = content.len(),
                        "Staged context file"
                    );
                }
...
async fn run_supervisor() {
    tracing::info!("Nexus Supervisor starting");
    ...
    loop {
        ...
        if self_compile {
            tracing::info!("Compiling nexus-agent");
        ...
        let current_exe = std::env::current_exe().unwrap_or_else(|_| PathBuf::from("nexus"));
...
/// Load persisted skills from .nexus-skills/ directory.
